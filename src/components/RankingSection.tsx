...
export type RankingSectionProps = {
  ...
  onWindowChange?: (window: RankingWindow) => void;
  limit?: number;
  showWindowTabs?: boolean;
  title?: string;
};

const EMPTY_COPY = 'Pas encore assez d’activité pour établir un classement.';
const LOW_CONFIDENCE_COPY = 'Classement basé sur une activité récente encore limitée — à prendre comme une indication, pas une valeur définitive.';
...
const RankingSection = ({
  ...
  const { items, confidence, isLoading, isError, refetch, isFetching } = useRankingList(window);
  const visible = limit ? items.slice(0, limit) : items;

  return (
    <section aria-labelledby="ranking-heading">
      <div className="flex flex-col sm:flex-row sm:items-center sm:justify-between gap-3 mb-5">
        <h2 id="ranking-heading" className="flex items-center gap-2 text-xl md:text-2xl font-bold font-outfit text-white">
          <Trophy className="h-5 w-5 text-manga-gold" aria-hidden="true" />
          {title}
        </h2>
        {showWindowTabs && onWindowChange && (
          <div role="tablist" aria-label="Période du classement" className="flex items-center gap-1 p-1 rounded-lg bg-white/[0.05] border border-white/[0.08] self-start">
            {(Object.keys(WINDOW_LABELS) as RankingWindow[]).map((option) => (
              <button
                key={option}
                type="button"
                role="tab"
      ...
      {isLoading ? (
        ...
        <div className="rounded-2xl border border-destructive/40 bg-destructive/10 p-6 text-center">
          <p className="text-sm text-white/60 mb-4">Impossible de charger le classement pour le moment.</p>
          <button
            type="button"
            onClick={() => refetch()}
            disabled={isFetching}
        ...
        <div className="rounded-2xl border border-dashed border-white/15 bg-white/[0.02] py-10 px-6 text-center">
          <Award className="h-8 w-8 text-white/30 mx-auto mb-3" aria-hidden="true" />
          <p className="text-sm text-white/50 max-w-sm mx-auto">{EMPTY_COPY}</p>
        </div>
      ) : (
        <>
          {!confidence.confident && (
            <p className="mb-4 rounded-lg border border-white/10 bg-white/[0.03] px-3 py-2 text-xs text-white/50" role="note">
              {LOW_CONFIDENCE_COPY}
            </p>
          )}
              ...
              >
                ...
                <div className="p-3">
                  ...
                  <div className="mt-1.5 flex items-center justify-between gap-2">
                    <span className={`text-[10px] font-bold uppercase tracking-wider ${item.legacyFallback ? 'text-white/40' : 'text-manga-gold'}`}>
                      {item.label}
                    </span>
                    {item.work.status && (
